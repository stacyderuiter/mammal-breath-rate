...
Because predictions were made for activity level 1 specifically, the data shown in these graphs *excludes* observations at activity levels other than 1 (so not all datapoints to which the model was fitted are shown in this plot -- only the activity level = 1 ones).
...
# plot data
gf_point(breath_rate ~ body_mass | habitat, color = ~location, data = breath_data |> filter(activity_level == 1),
         alpha = 0.3) |>
...
```{r}
gf_point(breath_rate ~ body_mass | habitat, data = breath_data |> filter(activity_level == 1),
